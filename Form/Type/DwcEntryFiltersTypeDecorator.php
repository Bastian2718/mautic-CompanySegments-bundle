 ...
 *
 * Extends buildFiltersForm to handle company_segments type natively.
 */
class DwcEntryFiltersTypeDecorator extends DwcEntryFiltersType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        // Use our extended buildFiltersForm that supports company_segments
        $formModifier = function (FormEvent $event, $eventName): void {
            $this->buildFiltersFormWithCompanySegments($eventName, $event, $this->translatorLocal);
        };
    ...
    /**
     * Extended version of FilterTrait::buildFiltersForm with company_segments support.
     *
     * This is a copy of the parent method with an additional case for 'company_segments'.
     */
    protected function buildFiltersFormWithCompanySegments(
        string $eventName,
        FormEvent $event,
        TranslatorInterface $translator,
        $currentListId = null
        ...
        switch ($fieldType) {
            case 'company_segments':
                ...
                $companySegmentChoices = $options['companySegments'] ?? $this->getCompanySegmentChoices();

                $customOptions['choices']                   = $companySegmentChoices;
        ...
        if (in_array($operator, ['empty', '!empty'])) {
            ...
            if (in_array($operator, ['regexp', '!regexp']) && $this->connection) {
                // Let's add a custom validator to test the regex
                $customOptions['constraints'][] =
                    new Callback(
